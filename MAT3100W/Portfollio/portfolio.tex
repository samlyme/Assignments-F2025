...
\begin{theorem}
    \label{prop:bijection-bidirectional}
...
\begin{definition}
    ...
    The truth value of a statement is equivalent to its contrapositive.
\end{definition}
...
\begin{example}
    \Cref{prop:bijection-bidirectional} can be proven bidirectionally by proving 
    \cref{lemma:bijection-cardinality} and \cref{lemma:cardinality-bijection}.

    \Cref{lemma:bijection-cardinality} can be proven directly. Assume that a 
    bejection \(f: A \rightarrow B\) exists between sets \(A\) and \(B\). 
...
\begin{example}
    ...
    We procede by contradiction by assuing that \(\sqrt{2} \in \mathbb{Q}\).
    So, we can write
    \[\sqrt{2} = \frac{a }{b }\]
