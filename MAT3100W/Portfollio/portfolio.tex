...
Another foundation of mathematics is the \textbf{set}, which gives us the ability 
to reason about collections of objects. In many ways, mathematical concepts can be 
reasoned about through sets.
...
Sets on their own are useful, however we can perform operations on sets to create 
new sets. These operations form a kind of ``set algebra''.
...
We can actually find that ``DeMorgan's Laws'' can be applied to sets, just as 
they were applied to logical statements. 
...
Similar to how we use truth tables to visualize the truth of logical 
statements, we can use \textbf{Venn diagrams} to visualize sets.
...
Now that we have defined sets, we can go on to define intuitive concepts like 
\textbf{numbers} formally. We can actually put numbers into sets that exhibit 
special properties.
...
Within the set of natural numbers \(\mathbb{N }\) and integers \(\mathbb{Z }\), 
we can create special subsets based on how they can or can't be divided. We call 
this ruleset \textbf{modular arithmetic}.
We have likely seen the notion of divisibility, before, however we find it useful 
to redefine them formally.
...
To generalize the idea of parity to other divisors, we must define \textbf{modular arithmetic}.
...
\begin{definition}
    ...
    Notice that the parity of a number is equivalent to its divisible by 2, and 
...
Venturing past the whole numbers, we find the \textbf{rational} and \textbf{irrational}
numbers. We can think of these as ``decimal numbers.'' We find that \textbf{irrational}
numbers have special properties. 
...
The \textbf{real numbers} are the ``main'' type of number we encounter in math. 
They encompass the natural, rational, and irrational numbers.
...
When dealing with collections of objects, we may want to calculate the number of 
possible ``arrangements.'' These arrangements are called \textbf{combinations} and 
\textbf{permutations}. They are calculated through the mathematical operation 
known as the \textbf{factorial}.
...
When discussing the notion of infinity, it is often useful to reason about them 
by viewing them as the size of an infinitely large set. The ``smallest'' of these 
infinities is known as the \textbf{Countability infinite}.
...
We find that some infinities may be larger than other infinities. Notably, any 
infinity larger than a countable infinity is known as \textbf{uncountably infinite}.
A fun fact is that we actually can't even ask the question of `how many infinities 
are there?' because the answer is ``axiomatically independent.''
...
We have discussed collections of objects, but not necessarily the relationships 
between the objects themselves, and how they can be expressed and reasoned about.
This leads us to explore the notion of \textbf{relations}. Relations tell us 
about how objects within a set are related. Then, \textbf{functions} give us a 
sense of how to relate objects across sets. The existence of a special type of 
function known as a \textbf{bijection} necessarily means the sets have equal size.
...
Some relations are special, and exhibit the same properties as equality, thus we 
call them equivalence relations.
...
Now that we have a solid understanding of some concepts in math, we may begin 
to prove some propositions we stated.

\subsection{Direct Proofs}

The most intuitive proof technique is the \textbf{direct proof}, which makes use 
of the standard rules of logic. 
...
With conditional statements, we find that they have their own algebraic manipulation
rules. Thus, we can transform conditionals into forms that are easier to prove. We find that 
the \textbf{contrapositive} of a conditional is equivalent to the original statement. 
Similarly, the \textbf{converse} and \textbf{inverse} are equivalent to each other.
Proving both the original or contrapositive and the converse or inverse is known as 
a \textbf{bidirectional} proof, and shows that the \textbf{antecedent} and the 
...
In the world of logic over sets, ie. making logical statements about collections 
of objects, we find it useful to either say that some amount of them exhibit a 
certain property. Typically, we will either say all elements exhibit a property, 
or that some element(s) exhibit a property. These statements can be made with the 
use of \textbf{quantifiers}.
...
To say something applies to all elements of a set, we use the \textbf{universal quantifier}.
...
To say something applies to at least one element of a set, we use the \textbf{existential quantifier}.
...
We can combine quantifiers to make precise statementsa about sets.
...
If we want to be even more precise than what is possible with quantifiers, we can 
perform an \textbf{existence and uniqueness proof} where we prove that not only is 
this property applied to an element of a set, that is also the only element of said set 
with that property. 
...
When we want to prove something about a very large set, and possibly a countably
infinite set, we can use mathematical induction. Although it seems circular at first, 
the logic does hold and this technique is fundamental to proving complex statements.
...
Lastly, if we are at a loss, we can prove a statement by \emph{disproving its opposite}.
If we assume the opposite of a statement, and run into a \textbf{contradiction} we 
find that the original statement must be true! This is because a contradiction 
is impossible in our world of math. 
...
The following notes are heavily based on the work \cite{coxeter_regular_polytopes}.

In our final project, we chose to explore thew idea of regular polytopes, a 
generalization of regular polygons and polyhedra into higher dimensional spaces. 
First, we define regular polytopes in 2D, aka. regular polygons, as polygons that 
have equal side lengths and equal interior angles. We can prove that there are 
actually an infinite number of regular polygons rather easily. finding that there 
exists a way to construct a \(n\)-sided polygon for all \(n \in \mathbb{N}\). 

\begin{proof}
Let \(n \in \mathbb{N }\). We can pick \(n\) equidistant points on a circle and 
connect them with lines to form the sides. Thus, we have a \(n\)-gon in 2D.
\end{proof}

However, this task is not possible in 3D. In 3D, we have to have sides that can 
``fold'' together to form a convex shape. More formally, the interior angle times 
the number of sides that meet at a vertex must be less than \(2 \pi\). Before proving 
this statement, I would like to define some useful notation. We say a polyhedra 
with \(p\)-gons as sides and \(q\) instances of these \(p\)-gons at each vertex 
is \(\{p, q\}\). 


\begin{proof}
    First, we pick some arbitrary \(p, q \in \mathbb{N}\). The interior angle of 
    \(\{ p \} \) is \(\frac{(p-2)\pi}{p}\). When we arange them around a point, 
    the sum of the interior angles is \(q \times \frac{(p-2)\pi }{p} = q(1-2/p)\pi\). This value 
    must be strictly less than \(2\pi\) for the polyhedra to be convex.
    ...
    Solving for this inequality, and finding only integer solutions where \(p, q > 2\),
    we find the solutions \(\{3,3\},\ \{3,4\},\ \{4,3\},\ \{3,5\},\ \{5,3\}\).
\end{proof}

Notice that to construct a regular \(n\)-D polytope, each of its sides are \(n-1\)-D
regular polytopes. We can see this by noticing that a cube has sides that are squares.
This idea generalizes to higher dimensions. 

\includegraphics[width=0.5\textwidth]{./images/polyhedra-2d-side.png}

We can also visualize higher dimensional objects via projections and nets. 
We can project 4D objects into 3D the same way we project the ``shadow'' of a 3D 
object onto a 2D surface.

\includegraphics[width=0.6\textwidth]{./images/projection-cube.png}

Another way to visualize higher dimensional objects is via its net. We do this by
``unrolling'' its sides.
\includegraphics[width=0.8\textwidth]{images/net4d.png}

To be very honest, this topic was pretty difficult for me to grasp. I tried my best,
and I feel as though I got a solid foundation, there is still a lot for me to 
learn in this area, and that I've barely scratched the surface. Although I `understand'
the proofs for the basic theorems, I find it difficult to find an intuitive foothold 
anywhere here. Nevertheless, this project has been extremely fun, and I feel as 
though I at least have some idea of how to reason about this specific area of math.
